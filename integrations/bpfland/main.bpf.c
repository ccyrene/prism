...
/* Prism identity-bus retrofit (prism/integrations/bpfland) — see below. */
...
/* ===== Prism identity bus (retrofit) =====================================
 * The SAME pinned prism_identity map the Prism daemon writes and the net/trace
 * consumers read. Here scx_bpfland reads it O(1) to decide latency priority from
 * a stable, cluster-assigned workload identity instead of (only) its sleep
 * heuristic. Map layout is byte-identical to prism/bpf/prism_maps.bpf.h. */
...
/* Thick-bus scheduling-policy sub-fields packed into `flags` — byte-identical to
 * prism/bpf/prism_maps.bpf.h (PRISM_SCHED_*). The daemon writes a per-identity
 * latency class derived from a pod label (prism.io/latency-class); bpfland maps
 * it to ITS OWN deadline knob (slice_lag) in task_dl() below, so this is
 * "bpfland, now identity-native", not a binary override. */
#define PRISM_SCHED_CLASS_SHIFT     8
#define PRISM_SCHED_CLASS_MASK      (0x7u  << PRISM_SCHED_CLASS_SHIFT)
#define PRISM_SCHED_WEIGHT_SHIFT    11
#define PRISM_SCHED_WEIGHT_MASK     (0x7Fu << PRISM_SCHED_WEIGHT_SHIFT)
#define PRISM_SCHED_WEIGHT_NEUTRAL  64u
...
/* This is a hand-copy of the canonical prism/bpf/prism_maps.bpf.h (the scx build
 * path is separate from the prism include path). These compile-time asserts make
 * the copy fail the build rather than silently drift from the frozen ABI. */
_Static_assert(sizeof(struct prism_identity) == 24, "prism_identity must stay 24 bytes (frozen ABI)");
...
/* One O(1) bus read: the task's workload identity value (or NULL if off-bus).
 * Keyed on the task's leaf cgroup id — the same key the daemon writes and the
 * eval harness seeds. The same identity also drives the net + tracing consumers.
 *
 * SCOPE (leaf vs pod): this is the task's LEAF cgroup id. The eval seeds that
 * exact leaf cgroup, so the lookup hits. A real K8s deployment keys the bus on
 * the POD cgroup id (a container task sits deeper), so production parity needs
 * the bounded pod-ancestor walk in libprism.bpf.h's prism_identity_of_current()
 * — kept as future work here, matching scx_prism.bpf.c. */
static __always_inline struct prism_identity *
prism_task_identity(const struct task_struct *p)
{
...
{
	/*
	 * PRISM thick-bus retrofit (FLOOR variant): consume the workload's identity
	 * AND its per-identity latency class from the shared bus and map them to
	 * bpfland's OWN deadline knob (slice_lag). The operator sets the class
	 * centrally via a pod label; prismd publishes it; we read it here in one O(1)
	 * lookup (the same identity also drives net + tracing). Unlike the
	 * replace-variant, we DEFER the policy to the return at the end of this
	 * function: bpfland's stock heuristic computes its deadline first (incl. its
	 * dsq_vtime/awake_vtime side-effects), then identity is applied as a
	 * ONE-SIDED floor on the result:
	 *
	 *   CRITICAL -> deadline = min(stock_dl, vtime_now - lag) in vtime space.
	 *               Identity guarantees AT LEAST a flat slice_lag boost — so a
	 *               CPU-bound latency-critical task the heuristic mislabels batch
	 *               is protected — but it NEVER under-cuts a sleepy task the
	 *               heuristic already boosts MORE than slice_lag (no regression in
	 *               case 1). Weight amplifies the floor above neutral only.
	 *   BATCH    -> a deadline PENALTY computed from vruntime alone, so a batch
	 *               hog that games the sleep heuristic to *look* interactive is
	 *               still deprioritized — identity can't be gamed.
	 *   NORMAL / UNSET / off-bus -> the stock heuristic deadline, unchanged.
	 *
	 * HONESTY GUARD: vanilla bpfland and bpfland+Prism are the SAME source/knobs;
	 * the ONLY difference is reading the bus, so any win is attributable to
	 * identity selection, not to hand-tuning bpfland.
	 */
	struct prism_identity *pid = prism_task_identity(p);
	__u32 prism_cls = pid ? ((pid->flags & PRISM_SCHED_CLASS_MASK) >> PRISM_SCHED_CLASS_SHIFT) : 0;
	__u32 prism_w   = pid ? ((pid->flags & PRISM_SCHED_WEIGHT_MASK) >> PRISM_SCHED_WEIGHT_SHIFT) : 0;
	...
	/*
	 * PRISM policy applied as a ONE-SIDED floor on the stock deadline @dl:
	 *   CRITICAL -> min(dl, vtime_now - lag) in vtime space. Smaller vtime =
	 *               earlier deadline = more priority; take whichever gives MORE
	 *               priority, so identity guarantees a slice_lag floor (protects a
	 *               CPU-bound task the heuristic mislabels batch) yet never reduces
	 *               a sleepy task's already-larger heuristic boost (no regression).
	 *   BATCH    -> deadline penalty from vruntime alone (gaming-proof), unchanged.
	 *   NORMAL / UNSET / off-bus -> the stock deadline @dl.
	 */
	if (prism_cls == PRISM_SCHED_CLASS_CRITICAL) {
		u64 lag = scale_by_task_weight(p, slice_lag);
		u64 floor;

		if (prism_w > PRISM_SCHED_WEIGHT_NEUTRAL)
			lag = lag * prism_w / PRISM_SCHED_WEIGHT_NEUTRAL;
		floor = vtime_now - lag;
		return time_before(floor, dl) ? floor : dl;
	}
	if (prism_cls == PRISM_SCHED_CLASS_BATCH)
		return p->scx.dsq_vtime + scale_by_task_weight_inverse(p, slice_lag);
